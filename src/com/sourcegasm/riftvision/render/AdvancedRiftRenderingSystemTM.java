...
public class AdvancedRiftRenderingSystemTM {
	...
	public static BufferedImage renderOculusVideoAndData(BufferedImage bufImg, int w, int h, NavData navdata) {
		...
		render.getGraphics().drawImage(layer1, CALIBRATION_X, CALIBRATION_Y - 10, w / 2 - CALIBRATION_X,
				h - CALIBRATION_Y * 2, null);
		render.getGraphics().drawImage(layer1, w / 2, CALIBRATION_Y - 10, w / 2 - CALIBRATION_X, h - CALIBRATION_Y * 2,
				null);

		render.getGraphics().drawImage(layer2_bent, CALIBRATION_X + LAYER2_OFFSET, CALIBRATION_Y - 10,
				w / 2 - CALIBRATION_X, h - CALIBRATION_Y * 2, null);
		render.getGraphics().drawImage(layer2_bent, w / 2 - LAYER2_OFFSET, CALIBRATION_Y - 10, w / 2 - CALIBRATION_X,
				h - CALIBRATION_Y * 2, null);
	...
	protected static int[] computeRGBArray(int[] pixels, int width, int height) {
		...
		for (int x = 0; x < width; x++) {
			for (int y = 0; y < height; y++) {
				...
				double srcR = (paramA * dstR * dstR * dstR + CALIBRATION2 * dstR * dstR + paramC * dstR + paramD)
						* dstR;
